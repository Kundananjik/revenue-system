    ...
    <div class="flex flex-1 overflow-hidden">
        <aside class="bg-white shadow-xl w-64 flex-shrink-0 hidden md:flex flex-col">
            <div class="p-6 flex-1">
                ...
                <nav class="space-y-2">
                    <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 rounded transition hover:bg-blue-50 {{ request()->routeIs('admin.dashboard') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-600' }}">Dashboard</a>
                    <a href="{{ route('admin.categories.index') }}" class="block px-4 py-2 rounded transition hover:bg-blue-50 {{ request()->routeIs('admin.categories.*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-600' }}">Categories</a>
                    ...
                    @if(auth()->user()->role === 'super-admin')
                        <div class="pt-4 mt-4 border-t border-gray-100">
                            ...
                            <a href="{{ route('admin.audit-logs.index') }}"
                               class="block px-4 py-2 rounded transition hover:bg-blue-50
                               {{ request()->routeIs('admin.audit-logs.*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-600' }}">
                                Audit Logs
                            </a>
                        </div>
                    @endif
        ...
        <aside x-show="open" 
            ...
            <div class="p-6">
                ...
                <nav class="space-y-2">
                    <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 rounded {{ request()->routeIs('admin.dashboard') ? 'bg-blue-100 text-blue-700' : '' }}">Dashboard</a>
                    <a href="{{ route('admin.categories.index') }}" class="block px-4 py-2 rounded {{ request()->routeIs('admin.categories.*') ? 'bg-blue-100 text-blue-700' : '' }}">Categories</a>
            ...
            <header class="bg-white shadow-sm border-b p-4 flex justify-between items-center sticky top-0 z-30">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="text-sm font-semibold text-red-500 hover:text-red-700 border border-red-200 px-3 py-1 rounded-md hover:bg-red-50 transition">
                            Logout
                        </button>
                    </form>
                </div>
